...
| 15+ | `Mission`+`MissionParticipant`, puis Wallet/Transaction, RpEvent, ClassifiedDocument, `FactionRelation` (diplomatie), et une interface staff commune pour choisir un joueur (sert à la fois `PlayerAssignment` et `Sanction`, autant la construire une seule fois). Côté pages publiques : `/departements`, `/wiki`, `/personnages`, `/actualites`, `/carte`, `/evenements` restent 100% statiques (aucun modèle Prisma), à migrer une par une si le staff doit pouvoir les gérer depuis le site. Escouades AIT non créées — aucune donnée existante à migrer, noms/composition à valider avant toute création (voir §9bis de la Lore Bible : ne pas inventer de canon sans validation) | À venir |
