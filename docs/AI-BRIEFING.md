...
- `docker-compose.yml` ne contient plus que Elasticsearch 8.15 (optionnel, fallback SQL sinon) — le service `postgres` en a été retiré le 2026-09-09 avec la désinstallation complète de Docker Desktop (voir note 4 plus bas). Tous les scripts (`scripts/*.ps1`) et l'UI (`RpLoginTerminal.tsx`, `DevConsole.tsx`) ont été nettoyés des références au Docker Postgres local à la même date.
...
4. **Docker Desktop a été désinstallé de cette machine le 2026-09-09** (winget `Docker.DockerDesktop`, + `wsl --unregister docker-desktop`), après une deuxième panne persistante du même type (socket AF_UNIX bloqué — cette fois `sailor-ingest.sock`, précédemment `dockerInference` ; mêmes méthodes de réparation retentées et à nouveau sans effet : `Remove-Item`, `wsl --shutdown`, arrêt de tous les process `docker*`). **Ne pas proposer de le réinstaller, ni de le remplacer par Podman/Rancher Desktop** (même classe de bug probable, VM WSL2 sous-jacente identique) — PostgreSQL est sur Neon (voir section 5) et Elasticsearch reste un optionnel à fallback SQL. Si un futur besoin réel de conteneurs se présente, en discuter avec l'utilisateur plutôt que de supposer qu'un autre moteur Docker-compatible réglerait le problème.
