...
export function GameEventRevisionHistory({ eventId }: { eventId: string }) {
  const [revisions, setRevisions] = useState<GameEventRevision[] | null>(null);
  const [expanded, setExpanded] = useState<string | null>(null);

  useEffect(() => {
    apiFetch<GameEventRevision[]>(`/events/${eventId}/revisions`)
      .then(setRevisions)
      .catch(() => setRevisions([]));
  }, [eventId]);
    ...
    <section className="mt-8 hologram-border rounded-lg p-6">
      ...
      {revisions === null ? (
        ...
        <p className="text-sm text-gray-600">
          Aucune révision — cet évènement n&apos;a pas encore été modifié depuis sa création.
        </p>
      ) : (
              ...
              <div key={r.id} className="rounded border border-metal/40">
                ...
                {isOpen && (
                  <div className="space-y-2 border-t border-metal/30 px-3 py-3 text-sm">
                    <p className="text-white">{r.title} — {r.type} — {new Date(r.date).toLocaleDateString("fr-FR")}</p>
                    <div>
                    ...
                    <div>
                      <p className="font-mono text-[10px] uppercase text-gray-600">Bilan / victimes</p>
                      <p className="whitespace-pre-wrap text-gray-400">{r.casualties || "—"}</p>
                    </div>
                    <div>
                      <p className="font-mono text-[10px] uppercase text-gray-600">Issue</p>
                      <p className="whitespace-pre-wrap text-gray-400">{r.outcome || "—"}</p>
                    </div>
                  </div>
                )}
              </div>
