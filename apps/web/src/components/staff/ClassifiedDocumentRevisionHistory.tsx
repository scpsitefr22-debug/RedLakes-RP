...
interface ClassifiedDocumentRevision {
  id: string;
  title: string;
  excerpt: string | null;
  content: string;
...
export function ClassifiedDocumentRevisionHistory({ documentId }: { documentId: string }) {
  const [revisions, setRevisions] = useState<ClassifiedDocumentRevision[] | null>(null);
  const [expanded, setExpanded] = useState<string | null>(null);

  useEffect(() => {
    apiFetch<ClassifiedDocumentRevision[]>(`/classified-documents/${documentId}/revisions`)
      .then(setRevisions)
      .catch(() => setRevisions([]));
  }, [documentId]);
    ...
    <section className="mt-8 hologram-border rounded-lg p-6">
      ...
      {revisions === null ? (
        ...
        <p className="text-sm text-gray-600">
          Aucune révision — ce document n&apos;a pas encore été modifié depuis sa création.
        </p>
      ) : (
              ...
              <div key={r.id} className="rounded border border-metal/40">
                ...
                {isOpen && (
                  <div className="space-y-2 border-t border-metal/30 px-3 py-3 text-sm">
                    <p className="text-white">{r.title} — {r.status}</p>
                    <div>
      ...
      <p className="mt-4 font-mono text-xs text-gray-600">
        Chaque sauvegarde archive l&apos;état précédent du document — rien n&apos;est jamais écrasé silencieusement.
      </p>
    </section>
  );
}
