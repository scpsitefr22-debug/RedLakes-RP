...
export function NewsRevisionHistory({ articleId }: { articleId: string }) {
  const [revisions, setRevisions] = useState<NewsRevision[] | null>(null);
  const [expanded, setExpanded] = useState<string | null>(null);

  useEffect(() => {
    apiFetch<NewsRevision[]>(`/news/${articleId}/revisions`)
      .then(setRevisions)
      .catch(() => setRevisions([]));
  }, [articleId]);
              ...
              <div key={r.id} className="rounded border border-metal/40">
                ...
                {isOpen && (
                  <div className="space-y-2 border-t border-metal/30 px-3 py-3 text-sm">
                    <p className="text-white">{r.title} — {r.category} — {new Date(r.date).toLocaleDateString("fr-FR")}</p>
                    <div>
